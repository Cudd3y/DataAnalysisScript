...
data <- data %>%
  ...
  mutate(Block_start = case_when(TargetNumber == 1 ~ TargetHighlighted, TargetNumber == 2 ~ lag(Block_start), TargetNumber == 3 ~ lag(TargetHighlighted, shift = 2), TargetNumber == 4 ~ lag(TargetHighlighted, shift = 3))) %>%
  select (Participant, Condition, TrialNumber, Block, TargetNumber, MovementTime, Block_start, TargetIsHit, TargetHighlighted, TargetClicked, PosX, PosY, Distance)
...
# Calculate Error_Rate for each block
...
data_per_block <- temp_get_start_time %>%
...
# Since movement time cannot be negative, the negative values are passed as NA so that they will not further be taken into account.
data_per_block$MovementTime[data_per_block$MovementTime < 0] <- NA
...
movement_time_per_trial <- aggregate(MovementTime ~ Condition + Participant + TrialNumber, data = data_per_block, sum)

# Calculate average (mean) error rate per trial
error_rate_per_trial <- aggregate(ErrorRate ~ Condition + Participant + TrialNumber, data = data_per_block, mean)
